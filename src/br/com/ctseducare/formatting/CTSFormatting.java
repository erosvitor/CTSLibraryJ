...
public class CTSFormatting {
    ...
    public static String formatPersonalName(String name) {
        ...
        str.append(String.valueOf(name.charAt(0)).toUpperCase());

        for (int i = 1; i < name.length(); i++) {
            if (name.charAt(i) == ' ') {
                i++;
                str.append(' ');
                str.append(String.valueOf(name.charAt(i)).toUpperCase());
            } else {
                str.append(String.valueOf(name.charAt(i)).toLowerCase());
            }
        }
        return str.toString();
    }
