...
public class CTSExtensoMoeda {
    ...
    private static final String[] dezenas = {"", "dez", "vinte", "trinta", "quarenta", "cinquenta", "sessenta", "setenta",
            "oitenta", "noventa"};
    private static final String[] centenas = {"", "cem", "duzentos", "trezentos", "quatrocentos", "quinhentos", "seiscentos",
            "setecentos", "oitocentos", "novecentos"};
    private static final String[][] milhares = {{"", ""}, {"", ""}, {"mil", "mil"}, {"milhão", "milhões"},
    ...
    public static String valorPorExtenso(String valor) {
        ...
        String numeroTmp;
        ArrayList<String> centenas = new ArrayList<>();
        StringBuilder inteiroPorExtenso = new StringBuilder();
        ...
        int indiceColuna;
        for (String centena : centenas) {
            if (!centena.equals("000")) {
                indiceColuna = (Integer.parseInt(centena) == 1 ? SINGULAR : PLURAL);
                inteiroPorExtenso.append(" ").append(centenaPorExtenso(Integer.parseInt(centena))).append(" ").append(milhares[indiceMilhar][indiceColuna]);
            }
            indiceMilhar--;
        }

        String tmp = inteiroPorExtenso.toString().trim();
        inteiroPorExtenso.setLength(0);
        inteiroPorExtenso.append(tmp);
        if ((totalCentenasCheias == 0) && (centenas.get(0).equals("1"))) {
            inteiroPorExtenso.append(" real");
        } else {
            inteiroPorExtenso.append(" reais");
        }

        if (decimalPorExtenso.equals(""))
            return inteiroPorExtenso.toString();
        else
